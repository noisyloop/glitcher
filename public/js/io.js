...
/*
 * io.js — file import, source/output canvases, and PNG / 3D export.
 *
 ...
 * output canvas, and exporting a PNG or a voxel GLB. It knows nothing about
 * sliders/effects; callers hand it ImageData and a provider for the export
 * frame.
 */

import { setStatus } from './ui.js';
import { imageToGlb } from './export3d.js';
...
const els = {
  ...
  export3dMode: document.getElementById('export3dMode'),
...
async function handleFile(file, onImport) {
  ...
  els.export3dBtn.disabled = false;
  setStatus('ready');
...
/**
 * Wire the 3D export button. The provider returns the frame to voxelize plus
 * the pixelate block size, so this module stays ignorant of sliders.
 * @param {() => Promise<{imageData: ImageData, blockSize: number} | null>} getExportSource
 */
export function initExport3D(getExportSource) {
  els.export3dBtn.addEventListener('click', async () => {
    setStatus('building 3d mesh…');
    const src = await getExportSource();
    if (!src) { setStatus('export failed'); return; }

    const mode = els.export3dMode.value === 'tiles' ? 'tiles' : 'blocks';
    let result;
    try {
      result = imageToGlb(src.imageData, { blockSize: src.blockSize, mode });
    } catch (err) {
      console.error('3d export failed', err);
      setStatus('3d export error');
      return;
    }
    if (!result) { setStatus('3d export failed — image is fully transparent'); return; }

    downloadBytes(result.bytes, `glitch3d_${Date.now()}.glb`, 'model/gltf-binary');
    const { grid, voxels, triangles } = result.stats;
    setStatus(`exported glb — ${grid} grid, ${voxels} voxels, ${triangles} tris`);
  });
}
